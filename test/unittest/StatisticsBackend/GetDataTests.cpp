...
TEST_P(get_data_no_data_tests, no_data)
{
    ...
    if (entity2.is_valid())
    {
        ...
        result = StatisticsBackend::get_data(
            ...
            bins,
            the_initial_time(),
            the_initial_time() + std::chrono::seconds(bins),
            statistic);
    ...
    for (size_t i = 0; i < result.size(); ++i)
    {
        ...
        // seconds.
        ASSERT_EQ(the_initial_time() + std::chrono::seconds(1) + std::chrono::seconds(i), result[i].first);
    }
}
...
TEST_P(get_data_with_data_tests, get_sum_data)
{
    ...
    switch (data_type)
    {
        ...
        case DataKind::DISCOVERY_TIME:
        {
            ...
            start = the_initial_time() + nanoseconds_to_systemclock_duration(4000);
            finish = the_initial_time() + nanoseconds_to_systemclock_duration(9000);
            ...
            start = the_initial_time() + nanoseconds_to_systemclock_duration(0);
            finish = the_initial_time() + nanoseconds_to_systemclock_duration(20000);
