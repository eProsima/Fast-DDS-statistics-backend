...
_INDEX_TOCTREE_PRO_NOTES_RE = re.compile(r'/rst/notes/(?P<name>v[\d.]+-pro)\s*$', re.MULTILINE)
_HEADING_VERSION_RE = re.compile(r'Version\s+(?P<version>\d+(?:\.\d+){1,3})')


def get_pro_release_version(repo_path, fallback='0.0.0'):
    """
    Get the Pro release version straight out of the current release notes page

    Branch-name and CI-env-var based detection were both tried and both broke
    in the real build environment (detached-HEAD checkouts and a CI system
    that doesn't set the GitLab variables assumed there). A separate
    PRO_VERSION file avoided the environment problem but just moved the
    "remember to bump this" burden to a second, redundant value that could
    drift from what the release notes actually say.

    Instead, same as Fast-DDS-Pro-Docs's parse_latest_version_from_notes():
    index.rst's toctree already has to be pointed at the current
    notes/vX.Y.Z-pro page as part of cutting a release (see docs/index.rst).
    Reading the version back out of that page's own "Version X.Y.Z" heading
    means there is exactly one place this number is ever written down.

    :param repo_path: The docs/ directory
    :param fallback: Value returned if index.rst or the page it points to
        can't be read, or neither contains the expected pattern
    :return: The Pro version as 'major.minor.patch[.build]'
    """
    try:
        with open(os.path.join(repo_path, 'index.rst'), 'r') as f:
            index_contents = f.read()
    except OSError:
        return fallback
    toctree_match = _INDEX_TOCTREE_PRO_NOTES_RE.search(index_contents)
    if not toctree_match:
        return fallback
    notes_page = os.path.join(repo_path, 'rst', 'notes', toctree_match.group('name') + '.rst')
    try:
        with open(notes_page, 'r') as f:
            notes_contents = f.read()
    except OSError:
        return fallback
    heading_match = _HEADING_VERSION_RE.search(notes_contents)
    return heading_match.group('version') if heading_match else fallback
...
pro_release = get_pro_release_version(script_path)
# The full version, including alpha/beta/rc tags.
release = u'{}.{}.{} / {} Pro'.format(
    versions['major'], versions['minor'], versions['patch'], pro_release)
# Sidebar version badge shows the same full X.Y.Z string as `release` - the short
# X.Y form was misleading (e.g. showed "2.5" for release "2.5.0").
version = release
