...
- |AlertKind::NEW_DATA-api|: Triggered when a statistics message with a ``DATA_COUNT`` greater than 0 is received.
...
In addition to their own trigger conditions, all alerts will also trigger if there are no entities that match the alert's entity filter.
