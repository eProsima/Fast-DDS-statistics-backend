        ...
        const DomainId domain_id = 0)
{
    ...
    for (const auto& topic : topics)
    {
        ...
        if (MONITOR_SERVICE_TOPIC == topic)
        {
            datareader_qos = eprosima::fastdds::statistics::dds::MONITOR_SERVICE_DATAREADER_QOS;
        }
        else
        {
            datareader_qos = eprosima::fastdds::statistics::dds::STATISTICS_DATAREADER_QOS;
        }
        ...
        std::string app_metadata)
{
    ...
    participant_qos_copy.properties().properties().emplace_back(
        "fastdds.application.id",
        app_id,
        "true");
    participant_qos_copy.properties().properties().emplace_back(
