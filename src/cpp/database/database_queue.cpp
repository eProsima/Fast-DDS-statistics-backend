        ...
        const R& item) const
{
    ...
    throw BadParameter("Unsupported Sample type and ExtendedMonitorServiceStatusData type combination");
}
...
void DatabaseDataQueue<ExtendedMonitorServiceStatusData>::process_sample()
{
    ...
    switch (front().second->data.status_kind())
    {
        case fastdds::statistics::StatusKind::PROXY:
        {
            ...
            try
            {
                if (item.second->entity_discovery_info.is_proxy_undiscovery)
                {
                    ...
                    try
                    {
                        auto guid_str = proxy_entity_handles_to_guid.at(item.second->instance_handle);
                        StatisticsGuid guid = serialize_guid(guid_str);
                        EntityKind entity_kind = database_->get_entity_kind_by_guid(guid);
                        auto matched_domain_entity = database_->get_entity_by_guid(entity_kind, guid_str);
                        ...
                        updated_entity = true;
                        // Remove from the stored proxy handles map
                        ...
                        database_->get_entity(entity)->discovery_source != DiscoverySource::DISCOVERY)
                {
                    ...
                    proxy_entity_handles_to_guid[item.second->instance_handle] =
                            to_string(item.second->entity_discovery_info.guid);
                }
            ...
            catch (const eprosima::statistics_backend::Exception&)
            {
                        ...
                        participant_enqueued.end())
                {
                    ...
                    proxy_entity_handles_to_guid[item.second->instance_handle] =
                            to_string(participant_discovery_info.guid);
        ...
        case fastdds::statistics::StatusKind::CONNECTION_LIST:
        {
            ...
            try
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().connection_list());
        ...
        case fastdds::statistics::StatusKind::INCOMPATIBLE_QOS:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().incompatible_qos_status());
        ...
        case fastdds::statistics::StatusKind::INCONSISTENT_TOPIC:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().inconsistent_topic_status());
        ...
        case fastdds::statistics::StatusKind::LIVELINESS_LOST:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().liveliness_lost_status());
        ...
        case fastdds::statistics::StatusKind::LIVELINESS_CHANGED:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().liveliness_changed_status());
        ...
        case fastdds::statistics::StatusKind::DEADLINE_MISSED:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().deadline_missed_status());
        ...
        case fastdds::statistics::StatusKind::SAMPLE_LOST:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().sample_lost_status());
        ...
        case fastdds::statistics::StatusKind::EXTENDED_INCOMPATIBLE_QOS:
        {
            ...
            {
                process_sample_type(domain, entity, item.second->data.local_entity(), sample,
                        item.second->data.value().extended_incompatible_qos_status());
